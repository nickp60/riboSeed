...
dir.create(args$out_folder)
 # subfolders <- subfolders[!grepl("123", subfolders)]
...
triplot_tree <- function(tree, df, name){
  ...
  writePlot(
    pl=multiplot(plotlist = list(p,lab,q), ncol=3, widths = c(.5,.1,.45)), 
    file=paste0(args$out_folder, "degenerate_", name, "_regions_plot"),
    w=8, h=4)
  bar <- ggplot(nsuc[nsuc$variable == "good",],
                aes(x=reorder(name, name), y=value ))+
    # geom_point(stat="identity", width = .80)+
    geom_hline(0, yintercept = 0, color="grey30") + 
    geom_boxplot(outlier.colour =  NA,color="grey40", fill="grey60",width=.7)+
    geom_point(position=position_jitter(width = .12, height = .02), shape=1)+
    scale_x_discrete(expand=c(.01,.01))+
    scale_y_continuous(expand=c(.01,.01), limits = c(-.05,7.5), breaks = 0:7)+
    theme(
      # axis.title.y = element_blank(),
      # axis.text.y = element_blank(), 
      axis.ticks = element_blank(), 
  ...
  writePlot(
    pl=bar, 
    file=paste0(args$out_folder, "degenerate_", name, "_regions_barplot"),
    w=4, h=4)
...
#  add sme hidden dummy points to force consistent widths
# expanded <- expand.grid(name=unique(tall$name), where=unique(tall$where), value=NA,
#                         sourcepath="dummy", variable="good", seed=1,
#                         total=7)
# print(expanded)
# str(expanded)
# tall <- rbind(tall, expanded)
tall$where <- factor(tall$where)
print(levels(tall$where))
levels(tall$where) <- c("a.", "b.")
bar_grouped <- ggplot(tall[tall$variable == "good",],
                      aes(x=name, 
                          # aes(x=reorder(name, name), 
                              color=where, fill=where,
                          y=value))+
  geom_boxplot(outlier.colour =  NA, width=.7, alpha=0.5)+
  # geom_boxplot(outlier.colour =  NA, color="grey40", fill="grey60",width=.5)+
  geom_point(position=position_jitter(width = .1, height = .02), 
             shape=1, color="black")+
  geom_hline(0, yintercept = 0, color="grey20") + 
  # facet_wrap(~where, ncol = 1, scales = "free_y", strip.position="right")+
  # facet_wrap(~where, ncol = 1, scales = "free_y")+
  # scale_x_discrete(expand=c(.01, .01))+
  scale_y_continuous(expand=c(.01, .01), limits = c(-.05, 7.5), breaks = 0:7)+
  theme(
    axis.ticks = element_blank(), 
    ...
    # strip.text = element_text( angle = 190),
...
writePlot(
  pl=bar_grouped, 
  file=paste0(args$out_folder, "degenerate_grouped_regions_barplot"),
  w=4, h=5)
