...
%%%%  This allows us to set the table font size
\usepackage{etoolbox}
...
% fix for ugly cmodern tilde 20160829
...
\\
\footnotesize{$^\ast$To whom correspondence should be addressed; E-mail: n.waters4@nuigalway.ie.}
}
\date{}
...
\begin{tabular}{p{2.6cm}p{11cm}}
  ...
  \hline
  GapFiller \cite{Nadalin2012} & local assembler using a hash-based method to produce ``long-reads'' from paired-end sequencing data, which can then be used in a \textit{de novo} assembly. \\
  \hline
  ...
  \hline
  GRabB\cite{Brankovics2016} & selective assembly tandem rDNA clusters and mitochondria\\
...
The ability to use rDNA flanking sequences to uniquely identify and place rDNA regions in their genomic context requires the flanking sequences to be distinct within the genome for each region. This is expected to be the case for most, if not all, prokaryotic genomes. To demonstrate this, rDNA and 1kb flanking regions were extracted from \coli{Sakai} \cite{Hayashi2001} (BA000007.2), in which the rDNA operons have been well characterized\cite{Ohnishi2000}. These regions were aligned with MAFFT\cite{Katoh2002}, and their consensus depth and Shannon entropy calculated for each position in the alignment\cite{Schmitt1997}.


In \coli{MG1655}, the first rDNA operon is located 363 bases downstream of \textit{gmhB} (locus tag b0200). Homologous rDNA regions were extracted from 25 randomly selected complete \coli{} chromosomes(S.Table \ref{table:accessions})  We identified the 20kb region surrounding \textit{gmhB} in each of these genomes, then annotated and extracted the corresponding rDNA operon and flanking sequences. These sequences were aligned with MAFFT, and the Shannon entropies and consensus coverage plotted.
...
The \textit{de fere novo} assembly bridged an average of 4 of the 7 rDNA regions in the artificial genome, while the \textit{de novo} assembly method failed to bridge any. To demonstrate that the choice of reference sequence determines the ability to assemble correctly through rDNA contigs, we ran riboSeed with the same \coli{} reads using  pseudocontigs derived from the \textit{Klebsiella pneumoniae HS11286} (CP003200.1) genome\cite{Liu2012}. The \textit{de fere novo} assembly with pseudocontigs from \textit{K. pneumoniae} bridged between 0 and 3 rDNAs, but also misassembled  several rDNA gaps (Figure \ref{fig:artificial}).

\begin{figure}[!h]
    \centering
    \hspace*{-2cm}\includegraphics[width=.99\textwidth]{PrettyMauve}
    \caption{Representative Mauve output describing the results of riboSeed assemblies of simulated reads generated by ART from the concatenated \textit{E. coli Sakai} artificial genome. From top to bottom: artificial reference chromosome; rDNA clusters (red bars); \textit{de fere novo} assembly (\coli reference), \textit{de novo} assembly (\coli reference), and \textit{de fere novo} assembly (K. pneumoniae reference). riboSeed with \coli reference assembles 4 of 7 rDNA regions, but the \textit{de novo} assembly recovers no rDNA regions correctly. riboSeed using a K. pneumoniae reference resolves a single rDNA region, but misassembles clusters.
}
...
\begin{figure}[!h]
  ...
  \caption{Variants of the artificial genome with mutation frequencies between 0 and 0.1 (ie, 100 substitutions per kbp). Neighbor-joining trees are shown, rooted by the original sequence. Correctly-assembled rDNAs were counted, and the distribution of results shown against the appropriate mutation frequency. Results are shown for models where substitutions are permitted (A) throughout the chromosome, and (B) only the flanking regions, the latter emulating the relative rate of substitution in rDNA and flanking regions. N=8.}
  \label{fig:degen}
\end{figure}
...
\subsubsection*{Simulated reads with \textit{E. coli Sakai} and \textit{K. pneumoniae} Genomes}
...
At either 20x or 50x coverage, \textit{de novo} assembly was unable to resolve a single rDNA cluster with any of the simulated read sets. \textit{de fere novo} assembly with riboSeed showed modest improvement to both the \coli{} assemblies. Increasing depth of coverage and read length improves assemblies.
...
\begin{table}[]
  ...
  \begin{tabular}{p{4.9cm}p{.95cm}p{.75cm}p{.75cm}p{2.43cm}p{.6cm}>{\color{black}}>{\hfill}p{.4cm}p{.2cm}p{.1cm}>{\color{black}}>{\hfill}p{.4cm}p{.2cm}p{.1cm}}
   ...
   & & & & Name & rDNAs & $\checkmark$ & skip & $\times$ & $\checkmark$ & skip & $\times$  \\
    \cmidrule{1-1}\cmidrule(l){2-2}\cmidrule(l){3-3}\cmidrule(l){4-4}\cmidrule(l){5-6}\cmidrule(l){7-9}\cmidrule(l){10-12}
    A. hydrophila SSU   & HiSeq   & 101   & 250   & ATCC 7966 & 10 & 0 & 10 & 0  & 4 & 6 & 0  \\
    \arrayrulecolor{lgray}\hline
    B. cereus VD118    & HiSeq   & 101   & 300  & ATCC 10987  & 12 & 0 & 12 & 0  & 1 & 11 & 0  \\
    \hline
    B. cereus ATCC 10987   & MiSeq   & 250   & 100  & NC7401 & 14 & 0 & 14 & 0  & 12 & 2 & 0  \\
    \hline
    B. fragilis HMW 615   & HiSeq   & 101   & 250   & 638R & 6 & 0 & 5 & 1  & 0 & 3 & 3  \\
    \hline
    R. sphaeroides 2.4.1 & HiSeq & 101 & 210 & ATCC 17029  & 4 & 0 & 4 & 0  & 1 & 3 & 0  \\
    \hline
    R. sphaeroides 2.4.1 & MiSeq & 251 & 100 & ATCC 17029  & 4 & 1 & 2 & 1  & 1 & 2 & 1  \\
    \hline
    S. aureus M0927 & HiSeq & 101 & 250 & USA300\_TCH1516 & 5 & 0 & 5 & 0  & 3 & 2 & 0  \\
    \hline
    V. cholerae CO 0132(5) & HiSeq & 100 & 110 & El Tor str. N16961  & 8 & 0 & 8 & 0  & 5 & 3 & 0  \\
    \hline
    V. cholerae CO 0132(5) & MiSeq   & 250   & 100   & El Tor str. N16961 & 8 & 0 & 8 & 0  & 4 & 4 & 0  \\
    \hline
    X. axonopodis pv. Manihotis UA323 & HiSeq   & 101   & 250   & pv. Citrumelo & 2 & 0 & 1 & 1  & 2 & 0 & 0 \\
    % \arrayrulecolor{black}
...
We thank Anton Korobeynikov for his helpful tips on optimizing SPAdes. Yoann Augagneur, Shaun Brinsmade, and Mohamed Sassi graciously provided access to the UAMS-1 genome sequencing data.
...
The artificial genome used for testing was constructed using the makeToyGenome.sh script included in the GitHub repository under riboseed/scripts. Briefly, the 7 rDNA regions from the \coli{Sakai} genome were extracted with 5kb flanking sequence upstream and downstream; these sequences were then concatenated to form a single, \ttilde100kb sequence containing the 7 rDNAs as well as their flanking context.
