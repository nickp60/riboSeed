...
repeats unresolvable by short read sequencing. It takes advantage of
...
The tests for the module can be found under the ``tests`` directory. I
run them with the unittests module. The tests assume the installation of
all the recommended tools.
...
NOTE: You must have riboSeed installed in a ``virtualenv`` to use this
script

There are a lot of commandline options for riboSeed, so it can help to
run the pipeline as script. The included ``riboBatch.sh`` script is an
example.

If you have access to a hpc, you can set up a python virtualenv, edit
the 7 fields in this script, make any other modifications needed to fit
your job, and submit with ``qsub``.

We recommend copying this file to your project directory, and
customizing it as needed.

.. |Build Status| image:: https://travis-ci.org/nickp60/riboSeed.svg?branch=master
   :target: https://travis-ci.org/nickp60/riboSeed
.. |PyPI version| image:: https://badge.fury.io/py/riboSeed.svg
   :target: https://badge.fury.io/py/riboSeed
.. |Coverage Status| image:: https://coveralls.io/repos/github/nickp60/riboSeed/badge.svg?branch=master
